...
const ShopFiltering = ({ filtersUa, filtersRu, filtersEn, filtersState, setFiltersState }) => {
  ...
  const placeholder = language === 'ua' ? 'КАТЕГОРІЇ' : language === 'ru' ? 'КАТЕГОРИИ' : 'CATEGORIES';
  const placeholder2 = language === 'ua' ? 'КОЛЬОРИ' : language === 'ru' ? 'ЦВЕТА' : 'COLORS';
  const placeholder3 = language === 'ua' ? 'ЦІНА' : language === 'ru' ? 'ЦЕНА' : 'PRICE';
  const placeholder4 = language === 'ua' ? 'СЕЗОН' : language === 'ru' ? 'СЕЗОНЫ' : 'SEASONS';
